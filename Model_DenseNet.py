...
def Model_DenseNet(train_data, Target, test_data, test_tar, BS=None, sol=None):
    ...
    model = dense_net(num_of_class=Target.shape[-1])
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
    model.fit(train_data, Target, steps_per_epoch=10, batch_size=BS, epochs=int(sol), validation_data=(test_data, test_tar) )
